...
impl App {
    ...
    fn draw_current_block(&self, frame: &mut Frame, area: Rect) {
        ...
        let status_icon = if is_over { "🔴" } else if max_pct >= 80.0 { "🟡" } else { "🟢" };
        ...
        let title = if is_over {
            format!(" ⚠️  {} LIMIT REACHED ", plan.name)
        } else if cb.is_active {
            format!(" ⏰ {} — 5h Block ", plan.name)
        } else {
